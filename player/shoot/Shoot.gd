...
func spawn_impact():
	if not $BulletSpread/RayCast.is_colliding():
		return
	if !green_paint:
		impact_instance = impact_red.instance()
	else:
		impact_instance = impact.instance()
	...
	var colliding_thing = $BulletSpread/RayCast.get_collider()
	if colliding_thing.is_in_group("enemyhit"):
		colliding_thing.get_parent().hit(green_paint)
